package be.ucll.se.team19backend.minio.service;

import io.minio.*;
import io.minio.messages.Item;
import org.springframework.stereotype.Service;

import be.ucll.se.team19backend.minio.config.MinioFetchException;
import be.ucll.se.team19backend.minio.config.MinioConfigurationProperties;
...
public class MinioService {
     ...
     * Simulate a folder hierarchy. Objects within folders (i.e. all objects which
     * match the pattern {@code {prefix}/{objectName}/...}) are not returned
     *
     ...
     */
    public InputStream get(String source) throws MinioException {
        try {
    ...
    public InputStream get(String source, String bucket) throws MinioException {
        try {
            GetObjectArgs args = GetObjectArgs.builder()
                    .bucket(bucket)
                    .object(source)
     ...
     * @throws
     * MinioException         if an error occur while fetching object metadatas
     */
     ...
     *
     * @param paths Paths of all objects with prefix. Objects names must be
     *              included.
     ...
     * @param headers Additional headers to put on the file. The map MUST be
     *                mutable. All custom headers will start with 'x-amz-meta-'
     ...
     * @throws MinioException if an error occur while uploading object
     */
    public void upload(String source, InputStream file) throws MinioException {
        try {
            PutObjectArgs args = PutObjectArgs.builder()
                    .bucket(configurationProperties.getBucket())
                    .object(source)
     ...
     * Upload a file to Minio
     *
     * @param source      Path with prefix to the object. Object name must be
     *                    included.
     * @param file        File as an inputstream
