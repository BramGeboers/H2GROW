...
export function AnimatedPlayer({
  ...
  const { materials, animations, scene } = useGLTF(
    "/models/peers/Character_Soldier.gltf"
  );
  ...
  useFrame(() => {
    if (linvel) {
      magnitude = Math.sqrt(linvel.x ** 2 + linvel.y ** 2 + linvel.z ** 2);
      if (magnitude > 0.3) {
        setAnimation("Walk");
        if (magnitude > 1.8) {
          setAnimation("Run");
    ...
    if (leftTriggerValue && magnitude < 0.3) {
...
useGLTF.preload("/models/peers/Character_Soldier.gltf");
